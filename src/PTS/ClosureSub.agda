...
module PTS.ClosureSub
  ...
  open import PTS.ClosureAlpha enum 𝒜 ℛ
  ...
  subRen : ∀ {Γ Δ x y s s' A σ} → x ∉ dom Γ → y ∉ dom Δ → Γ ⊢ A ∶ const s
         → Δ ⊢ A ∙ σ ∶ const s' → σ ∶ Γ ⇀ Δ → (σ ‚ x := var y) ∶ (Γ ‚ x ∶ A) ⇀ (Δ ‚ y ∶ A ∙ σ)
  subRen {Γ} {Δ} {x} {y} {s} {s'} {A} {σ} x∉domΓ y∉domΔ Γ⊢A:s Δ⊢Aσ:s' h = lemma2
    ...
    lemma2 : ∀ {z B} → (z , B) ∈ (Γ ‚ x ∶ A)
           → Δ ‚ y ∶ A ∙ σ ⊢ (σ ‚ x := var y) z ∶ B ∙ σ ‚ x := var y
    lemma2 {z} {B} _ with x ≟ z
    lemma2 {.x} {.A} (here refl) | yes refl =
      subst (_⊢_∶_ (Δ ‚ y ∶ A ∙ σ) (var y)) Aσ=Aσ<+x,y Δ,y:Aσ⊢y:Aσ
      where      
      x#A : x # A 
      x#A = c∉xs++ys→c∉xs (freshAsg x∉domΓ Γ⊢A:s)
      Aσ=Aσ<+x,y : A ∙ σ ≡ A ∙ σ ‚ x := var y
      Aσ=Aσ<+x,y = sym (updFresh {x} {A} x#A)
      Δ,y:Aσ⊢y:Aσ : Δ ‚ y ∶ A ∙ σ ⊢ var y ∶ A ∙ σ
    ...
    lemma2 {z} {B} (there z,B∈Γ) | no _ =
      subst (_⊢_∶_ (Δ ‚ y ∶ A ∙ σ) (σ z)) Bσ=Bσ<+x,y Δ,y:Aσ⊢σz:Bσ
      where
      Δ⊆Δ,y:A : Δ ⊆ (Δ ‚ y ∶ A ∙ σ)
  ...
  closureSub {Γ} {Δ} {_} {_} {σ} sub Δok
    Γ⊢λ[x:A]M:Π[y:A]B@(⊢abs {x} {y} {s₁} {s₂} {s₃} {A} {B} {M} Rs₁s₂s₃ Γ⊢A:s₁ h h₀) =
    ...
    Δ⊢Aσ:s₁ : Δ ⊢ A ∙ σ ∶ const s₁
    Δ⊢Aσ:s₁ = closureSub sub Δok Γ⊢A:s₁
    goal : ∀ z → z ∉ dom Δ
      → Δ ‚ z ∶ A ∙ σ ⊢ (M ∙ σ ‚ x := var x') [ x' := var z ]
                      ∶ (B ∙ σ ‚ y := var y') [ y' := var z ]
    goal z z∉domΔ =
      ...
      sub' : σ ‚ z' := var z ∶ (Γ ‚ z' ∶ A) ⇀ Δ ‚ z ∶ A ∙ σ 
      sub' = subRen z'∉domΓ z∉domΔ Γ⊢A:s₁ Δ⊢Aσ:s₁ sub
      Δ,z:Aσok : Δ ‚ z ∶ A ∙ σ ok
      Δ,z:Aσok = ⊢cons Δok z∉domΔ Δ⊢Aσ:s₁
      ih : Δ  ‚ z ∶ A ∙ σ ⊢ M [ x := var z' ] ∙ σ ‚ z' := var z
                          ∶ B [ y := var z' ] ∙ σ ‚ z' := var z
      ih = closureSub sub' Δ,z:Aσok (h z' z'∉domΓ)
      lemma1 : M [ x := var z' ] ∙ σ ‚ z' := var z ∼α (M ∙ σ ‚ x := var x') [ x' := var z ]
      lemma1 = 
        begin
        M [ x := var z' ] ∙ σ ‚ z' := var z
        ≈⟨ sym (composRenUpd {x} {z'} {M} {var z} z'∉fvM-x) ⟩
        M ∙ σ ‚ x := var z
        ...
        z'∉fvM-x : z' ∉ fv M - x
        z'∉fvM-x =
          c∉xs++ys→c∉ys {xs = fv A} (c∉xs++ys→c∉xs (freshAsg z'∉domΓ Γ⊢λ[x:A]M:Π[y:A]B)) 
      lemma2 : B [ y := var z' ] ∙ σ ‚ z' := var z ∼α (B ∙ σ ‚ y := var y') [ y' := var z ]
      lemma2 = 
        begin
        B [ y := var z' ] ∙ σ ‚ z' := var z
        ≈⟨ sym (composRenUpd {y} {z'} {B} {var z} z'∉fvB-y) ⟩
        B ∙ σ ‚ y := var z
        ...
        z'∉fvB-y : z' ∉ fv B - y
        z'∉fvB-y =
          c∉xs++ys→c∉ys {xs = fv A}
            (c∉xs++ys→c∉ys {xs = fv A ++ (fv M - x)} (freshAsg z'∉domΓ Γ⊢λ[x:A]M:Π[y:A]B))
    goal₀ : ∀ z → z ∉ dom Δ → Δ ‚ z ∶ A ∙ σ ⊢ (B ∙ σ ‚ y := var y') [ y' := var z ] ∶ const s₂
    goal₀ z z∉domΔ =
      ...
      Δ,z:Aσok : Δ ‚ z ∶ A ∙ σ ok
      Δ,z:Aσok = ⊢cons Δok z∉domΔ Δ⊢Aσ:s₁
      ih : Δ  ‚ z ∶ A ∙ σ ⊢ B [ y := var z' ] ∙ σ ‚ z' := var z ∶ const s₂
      ih = closureSub sub' Δ,z:Aσok (h₀ z' z'∉domΓ)
      lemma2 : B [ y := var z' ] ∙ σ ‚ z' := var z ∼α (B ∙ σ ‚ y := var y') [ y' := var z ]
      lemma2 = 
        begin
        ...
        z'∉fvB-y =
          c∉xs++ys→c∉ys {xs = fv A}
            (c∉xs++ys→c∉ys {xs = fv A ++ (fv M - x)} (freshAsg z'∉domΓ Γ⊢λ[x:A]M:Π[y:A]B)) 
  closureSub {Γ} {Δ} {_} {_} {σ} sub Δok (⊢app {x} {s} {M} {N} {A} {B}
    Γ⊢M:ΠxAB Γ⊢N:A Γ⊢[N/x]B:s) =
    ...
    lemma = 
      ...
      (B ∙ σ ‚ x := var x') [ x' := N ∙ σ ] 
      ∎ 
    Δ⊢σMσN:[σN/x']x'/x,σB : Δ ⊢ (M · N) ∙ σ ∶ (B ∙ σ ‚ x := var x') [ x' := N ∙ σ ]
    Δ⊢σMσN:[σN/x']x'/x,σB =
      ⊢app Δ⊢σM:Π[x:Aσ]x'/x,σB Δ⊢σN:σA (closAlphaAsg ∼ρs lemma Δ⊢σ[N/x]B:s)
  closureSub sub Δok (⊢conv Γ⊢M:A A=B Γ⊢B:s) =
    ⊢conv (closureSub sub Δok Γ⊢M:A) (compatConvSub A=B) (closureSub sub Δok Γ⊢B:s)
  closureSub {Γ} {Δ} {_} {_} {σ} sub Δok
    Γ⊢Π[y:A]B:𝒰@(⊢prod {y} {s₁} {s₂} {s₃} {A} {B} Rs₁s₂s₃ Γ⊢A:s₁ h) =
    ⊢prod Rs₁s₂s₃ Δ⊢Aσ:s₁ goal
    where
    y' : 𝒱
    y' = X (σ , fv B - y)
    y'#σ⇂fvB-y : y' #⇂ (σ , fv B - y)
    ...
    goal z z∉domΔ = closAlphaAsg ∼ρs lemma ih
      ...
      ih : Δ ‚ z ∶ A ∙ σ ⊢ B [ y := var z' ] ∙ σ ‚ z' := var z ∶ const s₂
      ih = closureSub sub' Δ,z:Aσok (h z' z'∉domΓ)         
      lemma : B [ y := var z' ] ∙ σ ‚ z' := var z ∼α (B ∙ σ ‚ y := var y') [ y' := var z ]
      lemma = 
        begin
        B [ y := var z' ] ∙ σ ‚ z' := var z
        ≈⟨ sym (composRenUpd {y} {z'} {B} {var z} z'∉fvB-y) ⟩
  ...
  subUnaryAux : ∀ {x s Γ N A} → x ∉ dom Γ → Γ ⊢ A ∶ s → Γ ∙∙ ι ⊢ N ∶ A ∙ ι
              → ι ‚ x := N ∶ (Γ ‚ x ∶ A) ⇀ Γ ∙∙ ι
  subUnaryAux {x} x∉domΓ Γ⊢A:𝒰 Γι⊢N:Aι {y} y∈domΓ,x:Aι with x ≟ y
  subUnaryAux {x} {s} {Γ} {N} {A} x∉domΓ Γ⊢A:s Γι⊢N:Aι {.x} (here refl) | yes refl =
    subst (_⊢_∶_ (Γ ∙∙ ι) N) Aι=Aι<+x,N Γι⊢N:Aι
    where
    x#A : x # A
    x#A = c∉xs++ys→c∉xs (freshAsg x∉domΓ Γ⊢A:s)
  ...
  subUnaryAux {x} {s} {Γ} {N} {A} x∉domΓ Γ⊢A:s Γι⊢N:Aι {y} {B} (there y,B∈Γ) | no _ =
    subst (_⊢_∶_ (Γ ∙∙ ι) (var y)) Bι=[N/x]B ιΓ⊢y:ιB
    where
    Γok : Γ ok
    Γok = validCxt Γ⊢A:s
    Γιok : Γ ∙∙ ι ok
  ...
  cut {Γ} {M} {N} {.(const s)} {B} {x} Γ,x:s⊢M:B Γ⊢N:s | s , inj₁ refl =
    ...
    Γok : Γ ok
    Γok = validCxt Γ⊢N:s
    Γ⊢s:s' : ∃ λ s' → Γ ⊢ const s ∶ const s'
    Γ⊢s:s' with validCxt Γ,x:s⊢M:B
    ... | ⊢cons {s = s'} _ _ Γ⊢s:s' = s' , Γ⊢s:s'
    ι,x=N:Γ,x:s→Γ : ι ‚ x := N ∶ (Γ ‚ x ∶ const s) ⇀ Γ
  ...
  unaryRen : ∀ {Γ x y A M B} → y ∉ dom Γ → Γ ‚ x ∶ A ⊢ M ∶ B
           → Γ ‚ y ∶ A ⊢ M [ x := var y ] ∶ B [ x := var y ]
  unaryRen {Γ} {x} {y} {A} {M} {B} y∉domΓ Γ,x:A⊢M:B =
    closureAlpha Γι,y:Aι∼Γ,y:A ∼ρ ∼ρ Γι,y:Aι⊢M[x=y]:B[x=y]
    where
    Γ,x:Aok : Γ ‚ x ∶ A ok
    Γ,x:Aok = validCxt Γ,x:A⊢M:B
    Γ⊢A:s : ∃ λ s → Γ ⊢ A ∶ const s
    Γ⊢A:s with Γ,x:Aok
    ...
    ι,x=y:Γ,x:A→Γ,y:Aι : (ι ‚ x := var y) ∶ (Γ ‚ x ∶ A) ⇀ (Γ ‚ y ∶ A) ∙∙ ι
    ι,x=y:Γ,x:A→Γ,y:Aι = subRen x∉domΓ y∉domΓι (proj₂ Γ⊢A:s) (proj₂ Γι⊢Aι:s) (identSub Γok)
    Γι,y:Aι⊢M[x=y]:B[x=y] : Γ ∙∙ ι ‚ y ∶ A ∙ ι ⊢ M [ x := var y ] ∶ B [ x := var y ]
